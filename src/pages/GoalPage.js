...
const Goal = () => {
  ...
  const [valor_inicial, setValor_inicial] = useState('');
  const [prazo, setPrazo] = useState('');
  const [valor_desejado, setValor_desejado] = useState('');
  ...
  const handleUpdateClick = (goal) => {
    ...
    setObjetivo(goal.objetivo);
    setValor_inicial(goal.valor_inicial);
    setPrazo(goal.prazo);
    setValor_desejado(goal.valor_desejado);
      ...
      <div className="panel-container">
        ...
        <table className="goal-table">
          ...
          <tbody>
            {goals.map((goal) => (
              <tr key={goal.id}>
                <td>{editingId === goal.id ? <input type="text" defaultValue={goal.objetivo} onChange={(e) => setObjetivo(e.target.value)} /> : goal.objetivo}</td>
                <td>{editingId === goal.id ? <input type="number" defaultValue={goal.valor_inicial} onChange={(e) => setValor_inicial(e.target.value)} /> : goal.valor_inicial}</td>
